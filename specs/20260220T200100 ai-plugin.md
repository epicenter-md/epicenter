...
**Superseded**: Phase 2 (client-side integration) was superseded by `specs/20260221T190252-ai-chat-tab.md` and subsequently by the per-conversation architecture in `specs/20260223T195800-background-ai-stream-on-switch.md`. The conceptual `useChat()` + `sseAdapter` client code was never implemented: the actual integration uses `createChat()` + `fetchServerSentEvents()` with per-conversation instances.
**Author**: AI-assisted

> **Topology note (2026-02-26)**: This spec was written before the hub/sidecar split. `createAIPlugin()` belongs exclusively on `createHubServer()`. The local sidecar (`createLocalServer()`) does NOT handle AI streaming: it handles workspace CRUD, extensions, actions, and persisted Y.Docs only. See `specs/20260222T195645-network-topology-multi-server-architecture.md` for the authoritative three-tier topology.
