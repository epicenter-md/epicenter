...
- [ ] 1. Server: Add env var API key fallback to `createAIPlugin()`
  ...
  **Acceptance Criteria**:
  ...
  Scenario: Env var API key works when header is absent
    ...
    Steps:
      1. Start server: OPENAI_API_KEY=sk-test-key bun packages/server/src/start-hub.ts &
...
- [ ] 3. Extension: Create server URL settings module
  ...
  **What to do**:
  ...
  - Define `serverUrl` storage item: `storage.defineItem<string>('local:server.url', { fallback: 'http://127.0.0.1:3913' })`
...
OPENAI_API_KEY=sk-test bun packages/server/src/start-hub.ts &
