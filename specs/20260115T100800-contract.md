...
**V2 will add:** Filesystem-based user schemas (`~/.epicenter/schemas/{schemaId}.json`)
...
~/.epicenter/
├── data/
│   └── {workspaceId}-{epoch}.ydoc    # Y.Doc persistence
├── schemas/
...
`~` is shorthand; use `os.homedir()` or platform API to resolve.
