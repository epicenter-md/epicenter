...
MIT: `packages/lens`, `packages/field`, `packages/ui`, and the toolkit-internal contracts they carry (`packages/data`, `packages/sqlite`, `packages/sync`, `packages/identity`, `packages/agent-protocol`, `packages/chat`). The embeddable developer toolkit.
