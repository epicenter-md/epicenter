...
Every ordinary row inherently owns one lazy collaborative body. The table does
not opt in or choose a format. A row handle exposes the one permanent content
binding from ADR-0135:

```ts
row.body.binding
```

The application chooses how its editor interprets the binding. The public API
exposes no free-standing documents, body identifier, text/rich-text mode,
generic Yjs document, arbitrary root, multiple named bodies, or conversion
operation. Empty bodies persist no bytes.

The row is the public identity and lifecycle aggregate. Its mutation vocabulary
is exactly:

```txt
create   complete fields required by this release, optional initial body update
update   field set/unset changes, a body update, or both
delete   end the row lifetime and cascade through fields and body
```

Creation mints a runtime row id and accepts no caller-supplied id. A deleted id
is never reused. `record` remains the internal name for the schema-opaque JSON
...
- `defineTable` declares fields only. Every ordinary row has the same latent
  body, with no declaration, contract, or format negotiation.
...
- JSON fields and KV retain authority-ordered replacement semantics. Only row
  bodies pay for interior CRDT merge.
...
- **Keep top-level parameterized documents.** Rejected because production
  ...
  deletion.
- **Declare a body kind per table.** Rejected because one fixed content binding
  is smaller than permanent per-table layout negotiation.
