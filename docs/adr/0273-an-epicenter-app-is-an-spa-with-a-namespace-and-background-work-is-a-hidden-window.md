...
- **Status:** Accepted
- **Date:** 2026-08-27
- **Amended by:** [ADR-0323](0323-background-work-runs-in-the-host-and-a-window-is-for-looking-at.md) at one decision and one refusal. Withdrawn: that background work is a window nobody is looking at, because a hidden window is suspended after about seven minutes and the measurement is in that record. Withdrawn: "no worker runtime", which a supervised first-party entry in the host plainly is. What stands: no second language, no per-application process, no new sandbox, no per-application route registration, and the reason the window was chosen in the first place, that an application's code does not change, which ADR-0323 keeps by moving the difference into the binding.
- **Superseded in part by:** the above. ADR-0322, which previously amended this record at who chooses, was superseded before it was built and decided nothing that survives.
