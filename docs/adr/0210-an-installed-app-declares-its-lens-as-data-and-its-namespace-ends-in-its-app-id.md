# 0210. An installed app declares its Lens as data, and its namespace ends in its app id
...
- **Amends:** [ADR-0179](0179-an-installed-app-is-an-inert-built-folder-admitted-through-one-static-artifact-boundary.md) at one clause: "installation still does not begin with a runtime manifest" is withdrawn for a Lens declaration only. Everything that clause was protecting stays: no permission grant, no installed-app registry, no publisher identity, no trust ceremony, and no per-app capability. Its refusal to install dependencies, run a build system, or read application source is untouched, because a Lens is none of those.
...
**An installed app declares its Lens as a JSON file in its own folder, and the
host reads it as data.**

### The declaration is a file beside the document

A candidate directory already holds `<id>/index.html`. It may also hold
`<id>/lens.json`, a Lens in the canonical JSON shape ADR-0168 defines. A folder
without one is admitted exactly as before and owns no namespace.
...
not validate is not a catalog member, the same disposition as a missing
`index.html`.
...
The Lens is frozen into the immutable generation at admission and selected once
at startup, exactly like every other byte of that generation. The host therefore
never depends on a running application to interpret a namespace, which is the
property that mattered. A generation is a release in ADR-0179's sense, so a Lens
inside one is as release-local as a Lens compiled into the binary.
...
- **Installation is still not a trust ceremony.** A Lens declares shape, never
  authority. It names no permission, no capability, no publisher, and no
  identity, and admission's answer to an invalid one is refusal rather than a
  prompt. ADR-0179's per-app permission refusal is untouched.
- **The install gesture is unchanged.** `catalog:publish` on a directory, active
  ...
  one stands.
- **`lens.json` is a fourth thing that can fail admission**, after an invalid id,
  a reserved id, and a missing `index.html`. It fails the same way: the folder is
  not a member. There is no partial admission where an app is launchable but its
  declaration was skipped, because a broken declaration is a broken app.
...
- **What this forecloses:** a manifest that declares anything other than a Lens,
  a runtime registration route an app calls to add a namespace while running, a
  Lens that changes without a new generation, and any admission-time prompt.
...
- **Let the app id be a reverse-domain string** so the id and the namespace are
  ...
  an app id names a directory under the one data root (ADR-0201) and a window
  label, and widening its grammar reopens both for a naming nicety.
- **Derive the namespace from the id** as `so.epicenter.<id>`, requiring no
  ...
  app.
- **Read the namespace from the app's source or build config.** Refused
  outright: ADR-0179's refusal to read application source is not the clause this
  record amends.
