...
- **Amended by:** [ADR-0349](0349-blobs-are-a-namespace-on-the-handle-addressed-by-id-and-stored-under-the-replicas-principal.md) at blob identity: withdrawn are "No blob identity crosses the boundary, ever. There is no `BlobId`" and the `audio.src = vocab.notes.blobUrl(note.id)` playback line. The recorder returns a `BlobId` at `stop`, the row's cell stores it, and playback goes through `epicenter.blobs.url(id)`. The row-that-fills rule, the `start` and `stop` semantics, the refusal of a `cancel` verb, and "a crash finishes it" stand.
