...
- **Amended by:** [ADR-0338](0338-every-change-a-folder-holds-has-an-answer.md) at the body's mechanism. This record's rule stands: a body reaches a store only when a person asked for it, on one row, deliberately. What is withdrawn is "as a whole-value replace" and the sentence that an application "must say that it replaces rather than merges": the table's codec rewrites the node the row already holds, so the node's identity and every binding to it survive, and it is the file's whole text winning rather than a merge. The Decision's headline that a body "does not read back" is withdrawn with it.
