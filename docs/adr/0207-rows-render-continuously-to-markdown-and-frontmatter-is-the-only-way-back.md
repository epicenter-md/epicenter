...
- **Partly built.** `defineTable`'s `body` key, the renderer, the receipt store, the scan, `push`, and `status` all exist and are tested end to end against a real replica (`apps/epicenter/src/folder/`). **Unbuilt:** nothing calls them. The host does not start the renderer at boot, and `status` and `push` are not yet actions (ADR-0021).
