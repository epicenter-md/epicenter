...
- **Partly built.** `defineTable`'s `body` key, the renderer, the receipt store, the scan, `push`, and `status` exist, are tested end to end against a real replica (`apps/epicenter/src/folder/`), and the host starts the renderer at boot (`main.ts`). **Unbuilt:** `status` and `push` are not yet actions (ADR-0021), so the folder is currently read-only in practice.
