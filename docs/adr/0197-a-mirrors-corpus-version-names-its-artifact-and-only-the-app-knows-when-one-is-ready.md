...
- **Amended by:** [ADR-0317](0317-local-mail-is-an-epicenter-application-without-a-standalone-cli.md) at Local Mail, and only there. Withdrawn for it: the versioned filename, the retained predecessor, and reclamation. Local Mail no longer names a file, because it opens `epicenter.openSqlite('mail')` and never learns where that lands, so a shape change is `mailbox.reset()` and a fresh pull rather than a second artifact beside the first. What stands everywhere is the reason: a mirror is disposable, opening one never destroys data as a side effect, and only the app knows when one is ready. Local Books is untouched.
