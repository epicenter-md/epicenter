...
- **Amended by:** [ADR-0317](0317-local-mail-is-an-epicenter-application-without-a-standalone-cli.md) at Local Mail, and only there. Withdrawn for it: the versioned filename, the retained predecessor, and reclamation. Local Mail no longer names an artifact, because it opens a database by name and never learns where that lands, so a shape change deletes the copy and pulls again rather than leaving a second artifact beside the first. How it does that moved once more in [ADR-0319](0319-local-mail-is-device-local-and-its-storage-splits-by-lifetime.md): the copy is one file per account, its shape is stamped inside it, and a build that does not understand the shape unlinks the file. What stands everywhere is the reason: a mirror is disposable, opening one never destroys data as a side effect, and only the app knows when one is ready. Local Books is untouched.
- **Amended by:** [ADR-0247](0247-an-app-that-keeps-a-local-copy-of-a-providers-data-owns-its-file-lifecycle.md) at the primitive's home: everything below holds, but each app owns its own copy of it in `src/db-file.ts` rather than importing `@epicenter/sqlite/bun-mirror`.
