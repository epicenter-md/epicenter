...
- **Status:** Accepted
- **Date:** 2026-08-01
- **Amended by:** [ADR-0317](0317-local-mail-is-an-epicenter-application-without-a-standalone-cli.md) at the ownership mechanism. Withdrawn: the per-account lock file and the claim that it excludes a second writing process. There are no separate processes left to exclude, so the capability `reconcileAccount` requires is an in-surface claim, and two surfaces racing are held instead by idempotent Gmail writes, sequence-matched retirement, and a `MirrorBusy` outcome. The pass shape, the two failure shapes, and the refusal of a dead-letter tier are unchanged.
