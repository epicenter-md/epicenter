...
bootstrap then mutate the optimistic table and replay pending commands. One
...
current-state projections, release table views, and bootstrap scratch are not
canonical tables.
...
state; deterministic lifecycle folding makes it inert when its target row died.
No replay path writes pending values into confirmed storage.
...
- Losing projections or disposable bootstrap scratch loses no unique user data.
...
- **Persist bootstrap staging in the workspace schema.** Rejected by ADR-0136:
  incomplete install data is disposable scratch, not canonical state.
