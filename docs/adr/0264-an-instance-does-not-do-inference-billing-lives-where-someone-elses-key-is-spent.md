...
- **`rateLimit` changes owner rather than dying.** Its only call sites were the four self-host mounts, so the first read was that it becomes deletable. That was wrong: Cloud's chat settles AFTER the call with `overageBehavior: "overflow"`, so N concurrent calls at exhaustion each run before any settles, and the billing spec already names the bound as per-call cost times the rate limit. Cloud was never mounting one, so that stated bound was not true. It is mounted on Cloud's inference and transcription gateways now, per principal. Per-isolate on Cloudflare and therefore approximate, which suits bounding a one-time overshoot and is not a sustained-abuse defense.
...
- **Transcription goes with chat, not after it.** One `Connection` already drives both: the client appends `/chat/completions` or `/audio/transcriptions` to the same base URL, `transcribe()` takes the same `ResolvedConnection` as the chat engine, and both Vocab and Whispering already resolve their STT model through the connection registry. `/v1/audio/transcriptions` is a standard OpenAI endpoint that Groq, Speaches, whisper.cpp, and LiteLLM serve, and the desktop host runs local models on device. So a self-hoster points STT at an endpoint exactly the way they point chat, and no new client primitive is needed.
