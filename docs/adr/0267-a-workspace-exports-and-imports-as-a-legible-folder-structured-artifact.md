...
- **Built:** the artifact, both directions as pure functions. `packages/data/src/artifact/` assembles the files from `data.stored()` and the `file` codecs, and reads them back into one envelope through the same codecs; `@epicenter/data/artifact` publishes both, and the round trip is tested end to end through real stores.
