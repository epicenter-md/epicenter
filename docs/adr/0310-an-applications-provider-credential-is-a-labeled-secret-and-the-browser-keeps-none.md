...
- **Status:** Accepted
- **Date:** 2026-08-31
- **Built**, less two things. The three verbs, the keychain leaf, the tab-memory leaf, and Local Mail's browser build all exist. Unbuilt: the hidden window that would make desktop synchronization background rather than while-open, and the synchronizing half of the account registry. `epicenter.openData` opens a device-local document, so "the account list synchronized; the credentials did not" is half true today: the credentials do not, and neither does the list. Making it true needs the account overload of `openDatabase` and therefore the signed-in principal the host brokers, which is a decision about which authority an application's own data belongs to.
