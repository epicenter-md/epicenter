...
- **Status:** Accepted
- **Date:** 2026-08-31
- **Built**, less two things. The three verbs, the keychain leaf, the tab-memory leaf, and Local Mail's browser build all exist. Unbuilt: starting the application's window hidden at boot. How that is chosen is decided by [ADR-0322](0322-a-person-keeps-applications-current-and-a-hidden-window-is-the-same-window.md). Synchronization is already background rather than while-open, because closing an application window hides it and the page keeps running; what a hidden start adds is that a person who has not launched the application since quitting Epicenter still has one that is current. The registry half is settled rather than pending: ADR-0319 withdrew it, so the list does not synchronize and is not going to.
