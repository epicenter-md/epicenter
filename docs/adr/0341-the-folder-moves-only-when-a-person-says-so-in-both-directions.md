...
- **Unbuilt:** all of it. `packages/data/src/artifact/checkout.ts` today ends `push` with `pull({ discardEdits: true })`, which renders the whole store over the folder; `pull` refuses a dirty folder with `WorkingCopyDirty` and takes `discardEdits` instead of a plan; a file the push cannot read is a `PlannedDiscard` whose files are overwritten, and `row-gone` deletes the file outright because the re-render never names its path.
