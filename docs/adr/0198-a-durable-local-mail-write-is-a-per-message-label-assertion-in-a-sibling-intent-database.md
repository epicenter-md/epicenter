...
- **Status:** Accepted
- **Date:** 2026-08-01
- **Amended by:** [ADR-0317](0317-local-mail-is-an-epicenter-application-without-a-standalone-cli.md) at the word "sibling". The intent store is a second database opened through the scoped handle rather than a file beside the mirror, and its rows are partitioned by the account id Epicenter Data minted rather than living in a per-account directory. The separation the title is about is stronger for it: a cache reset holds no handle to the intent database and cannot reach it. Everything the record decides about what a durable write IS is unchanged.
