...
- **Status:** Accepted
- **Date:** 2026-07-17
- **Superseded by:** [ADR-0212](0212-epicenter-replicates-cells-and-a-cells-version-carries-no-identity.md) (`Proposed`). This record was already superseded by ADR-0163, which ADR-0212 also supersedes, so the link is restated here rather than left to a broken chain. Withdrawn: the settled-watermark lifecycle and the caller-visible watermark itself, replaced by a dirty flag on each cell and a cursor the replica never stores per cell.
