...
//! To prove (or kill) each optimization with real numbers instead of vibes,
//! the budget components log their wall time on the `whispering::timing`
//! target when `WHISPERING_TIMING` is set in the environment. Unset (the
//! default, including release builds), every helper is a branch-and-return:
//! no clock read, no allocation, no log line.
//!
//! Read the spans with:
//!
//! ```sh
...
//! ```
//!
//! The cold model-load number (the signal that decides PR 2 vs PR 3 in
//! `specs/20260617T170000-desktop-audio-pipeline-greenfield.md`) and the
//! inference number are emitted from `transcription::model_cache`; the
//! finalize, persist, decode, and Opus spans live next to the code they
//! measure.
