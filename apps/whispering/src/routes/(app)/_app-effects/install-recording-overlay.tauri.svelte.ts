...
/** Drive the native recording overlay window for the app's lifetime. */
export function installRecordingOverlay(): void {
	...
	// The two overlay event subscriptions resolve asynchronously, so an unmount
	// can land before a `listen` settles; the guard makes a late-resolving
	// unlistener run immediately instead of leaking.
	onMount(() => {
