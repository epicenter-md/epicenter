...
Reference single-partition **instance** (ADR-0075, amended by ADR-0092): one operator-supplied bearer (`INSTANCE_TOKEN`), one literal `instance` principal, one `principals/instance` partition. Composes `@epicenter/server` with `createEnvTokenResolver(token)` and `requireBearerPrincipal`. Two runtimes off one set of library surfaces: an off-Cloudflare Bun entry (`server.ts`, blessed) and a Cloudflare Worker (`worker/index.ts`). They authenticate identically because the operator supplies the secret either way, and they serve the same surfaces except attach (`/attach`, `/attach/grants`, `/attach/hosts`), which is Bun-only: the transport is `Bun.serve`'s WebSocket handler and the per-device grants are an in-process store. `runtime-profile.test.ts` declares that one divergence and fails on any other. "Solo" vs "shared" is only how many people hold the token, never a mode.
