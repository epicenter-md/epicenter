...
const PROFILE: Surface[] = [
	...
	},
	{
		...
		worker: 'absent',
		bun: 'absent',
		why: 'An instance does not do inference (ADR-0264). It is identity, sync, and storage, and holds no provider house key. Inference is a device-local connection the client points wherever the operator likes.',
	},
	{
		...
		worker: 'absent',
		bun: 'absent',
		why: 'Same as inference (ADR-0264): one Connection drives both chat and STT, so a client points transcription at a Speaches box, Groq, or a local model with no gateway here.',
	},
	{
