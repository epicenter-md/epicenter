/**
 * Where Local Mail's database lives on disk, and what it is called.
 *
 * The mailbox copy is disposable by construction: everything in it came from
 * Gmail and can come from Gmail again. So a change to what this build stores is
 * not a migration, it is a different filename. `mail.v<version>.db` carries its
 * version in its name, a successor is built beside its predecessor, and opening
 * never drops, truncates, migrates, or unlinks anything. A reader still holding
 * the old file keeps reading it while the new one fills (ADR-0197).
 *
 ...
 * constant, the schema, ingestion, cursors, "is this file filled yet", locking,
 * file permissions, and when a predecessor may be deleted all stay with the
 * app.
 */
...
/** One version of the database present in an account's directory. */
export type DbFileVersion = {
	version: number;
	/** The account directory joined with this version's filename. */
	path: string;
...
export type DbFile = {
	 ...
	 * a sibling `intent.db` or `credentials.json` is unreachable. It never
	 ...
	 * reader keeps answering from a mailbox that silently rolled back; on Windows
 ...
 * reconciler retrying a broken file would accumulate.
 */
