 ...
 *
 * The golden fingerprint below is pinned deliberately, and it is pinned to the
 * SAME value as `apps/local-books/test/mirror.test.ts`. That is the point: the
 * two apps hold byte-identical copies of `src/mirror.ts` until the primitive is
 * extracted into a package, and one hash computed over one sample declaration is
 * what proves neither copy's canonical serialization has drifted. If the
 * serialization ever does change, every mirror in the field renames its artifact
 * and re-pulls, so that must be a decision someone makes on purpose (by bumping
 * the format tag) rather than a refactor's side effect.
 */
...
import { type DeclarationValue, defineMirror } from './mirror.ts';

function tempDir() {
	const dir = mkdtempSync(join(tmpdir(), 'local-mail-mirror-test-'));
...
describe('opening', () => {
	...
	test('read-only reports an absent artifact instead of conjuring one', () => {
		const tmp = tempDir();
		const site = widgets().at(join(tmp.dir, 'you@example.com'));
...
describe('reclaim', () => {
	test('deletes one predecessor and its sidecars, and nothing else', () => {
		...
		// The siblings Local Mail keeps beside the mirror. Reclaim is scoped to the
		// filename grammar precisely so the sync lock and the OAuth refresh token
