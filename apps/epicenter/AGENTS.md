...
- `src/applications.ts` composes what Home lists as launchable: compiled applications plus the selected catalog generation's members, in one `{ id, title }` shape served at `/api/apps`. Home renders one row and calls one native verb (`launch_application`), which dispatches privately to `request_surface` or an `app-` window. That list is not "the catalog": `catalog` keeps its ADR-0179 meaning, the immutable generation of admitted folders. Do not surface "compiled versus admitted" to a user, do not add a second launch command, and do not reuse ADR-0181's app-facing `openApp(appId)` name for it.
