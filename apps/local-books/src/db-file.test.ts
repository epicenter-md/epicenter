...
function tempDir() {
	const dir = mkdtempSync(join(tmpdir(), 'books-db-file-test-'));
	return { dir, cleanup: () => rmSync(dir, { recursive: true, force: true }) };
}

const at = (directory: string, version = 1) => dbFileAt({ directory, version });

test('names the file books.v<version>.db', () => {
	expect(at('/tmp/site', 5).path).toBe('/tmp/site/books.v5.db');
	expect(at('/tmp/site', 12).path).toBe('/tmp/site/books.v12.db');
});

describe('opening', () => {
	test('creates the current version, and only that one', () => {
		const tmp = tempDir();
		const file = at(join(tmp.dir, 'realm-1'), 5);
...
describe('versions()', () => {
	test("lists only this app's database files, never a sibling it owns", () => {
		...
		writeFileSync(join(tmp.dir, 'books.db'), '');
		writeFileSync(join(tmp.dir, 'books.v0.db'), '');
		writeFileSync(join(tmp.dir, 'books.v05.db'), '');
		writeFileSync(join(tmp.dir, 'books.vx.db'), '');
		writeFileSync(join(tmp.dir, 'mail.v5.db'), '');

		expect(file.versions().map((entry) => entry.path)).toEqual([
			join(tmp.dir, 'books.v5.db'),
		]);
		tmp.cleanup();
	});
	...
	test('opens no SQLite handle, so an unreadable file is still listed', () => {
		const tmp = tempDir();
		writeFileSync(join(tmp.dir, 'books.v4.db'), 'not a database at all');
		expect(at(tmp.dir, 5).versions()).toEqual([
			{ version: 4, path: join(tmp.dir, 'books.v4.db'), current: false },
		]);
		tmp.cleanup();
	});
});
...
describe('deleteOlderVersions', () => {
	test('deletes every lower version and its sidecars, and nothing else', () => {
		...
		// filename pattern precisely so a sync lock and an OAuth
		// refresh token are unreachable from it.
		writeFileSync(join(tmp.dir, 'lock.db'), '');
		writeFileSync(join(tmp.dir, 'credentials.json'), '{}');
		...
		expect(readdirSync(tmp.dir).sort()).toEqual(
			['credentials.json', 'lock.db', 'books.v5.db'].sort(),
		);
		tmp.cleanup();
	});
