...
- Read Epicenter store data with `db.<table>.subscribe(listener)` feeding `$state.raw`. The listener fires after the projection commits (ADR-0221), so the handler re-reads `db.<table>.list()` and reassigns; `$derived` does the filtering and sorting. Exemplar: `apps/honeycrisp/src/routes/state/notes.svelte.ts`.
