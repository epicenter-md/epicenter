...
- Read Epicenter store data through `fromWorkspace` from `@epicenter/svelte`: it mirrors an opened workspace's declared `tables`/`kv`, makes every read verb (`rows`, `nonconforming`, `list`, `get`, `ids`, `document`) reactive, and passes writes through. Subscriptions are ref-counted to effect usage, so nothing is disposed by hand and unread tables cost nothing. Do not hand-roll `db.<table>.subscribe(listener)` feeding `$state.raw` for table reads any more; `$derived` over `table.rows` does the filtering and sorting. Adapter: `packages/svelte/src/from-workspace.svelte.ts`; app exemplar: `apps/honeycrisp/src/lib/honeycrisp/notes.svelte.ts`.
