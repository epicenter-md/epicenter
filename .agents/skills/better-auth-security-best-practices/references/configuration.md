...
This reference covers Better Auth's own security surface: secrets, rate
limiting, CSRF, trusted-origin configuration, sessions, cookies, OAuth token
handling, IP resolution, audit hooks, and a full worked configuration. Values
here are upstream defaults and can change between versions; confirm decisive
ones against installed types or DeepWiki.
...
export const auth = betterAuth({
  ...
  databaseHooks: {
    session: {
      create: {
        after: async ({ data, ctx }) => {
          auditLog.info("session.created", { userId: data.userId });
        },
      },
    },
    user: {
      update: {
        after: async ({ data, oldData }) => {
          if (oldData?.email !== data.email) {
            auditLog.info("email.changed", { userId: data.id });
          }
        },
      },
    },
