...
const PROFILE_DEFAULTS: Record<'smoke' | 'full', Profile> = {
	...
	full: {
		facts: 1_000_000,
		initialPayloadBytes: 512 * 1024 * 1024,
		repetitions: 5,
		steadyWrites: 10_000,
		pointReads: 2_000,
		traversals: 16,
...
Options:
  --profile smoke|full       Defaults to smoke. Full uses 1,000,000 facts and
                             approximately 512 MiB of initial payload.
  --facts <positive-int>     Override the number of current facts.
  --initial-payload-bytes <int>
                             Override the target initial payload bytes.
	...
	for (let index = 0; index < args.length; index += 1) {
		...
		switch (flag) {
			...
			case '--facts':
				...
				break;
			case '--initial-payload-bytes':
				options.initialPayloadBytes = parsePositiveInteger(flag, value);
				break;
			case '--repetitions':
...
function checkTemporarySpace(options: Options) {
	const requiredBytes = Math.ceil(
		Math.max(
			256 * 1024 * 1024,
			options.initialPayloadBytes * 3 + options.facts * 1024 + 64 * 1024 * 1024,
		),
	);
	...
	return {
		...
		requiredBytes,
		estimateFormula:
			'max(256 MiB, 3×initial payload target + 1 KiB per fact + 64 MiB bounded WAL/headroom)',
	};
}
...
function makePayload(
	...
	if (address.kind === 'value') {
		...
		return JSON.stringify(
			prefix +
				repeatToLength(
					(mixed >>> 0).toString(36),
					Math.max(0, payloadBytesPerFact - prefix.length - 2),
				),
		);
	}
	...
	return JSON.stringify({
		body: repeatToLength(
			(mixed >>> 0).toString(36),
			Math.max(0, payloadBytesPerFact - fixed),
		),
		ordinal: index,
		phase,
		active: true,
...
function runOne(
	...
	try {
		...
		const payloadBytesPerFact = Math.max(
			8,
			Math.floor(options.initialPayloadBytes / options.facts),
		);
		...
		for (let start = 0; start < options.facts; start += acquisitionBatchSize) {
			...
			for (let index = start; index < end; index += 1) {
				const fact = initialFact(index, options, payloadBytesPerFact);
				initialPayloadBytes += Buffer.byteLength(fact.payload ?? '');
				batch.push(fact);
			}
		...
		const valueProbe = initialFact(0, options, payloadBytesPerFact);
		...
		const terminalProbe = initialFact(
			terminalIndex,
			options,
			payloadBytesPerFact,
		);
		...
		if (owner === 'replica') {
			...
			for (
				...
				for (let item = 0; item < ADDRESS_BATCH; item += 1) {
					...
					if (isValueWorkloadOrdinal(ordinal, totalAddresses, targetValues)) {
						...
						mixed.push({
							...initialFact(valueIndex, options, payloadBytesPerFact),
							payload: makePayload(
								valueIndex,
								options.facts,
								options.dataSeed,
					...
					usedRows.add(index);
					const probe = initialFact(index, options, payloadBytesPerFact);
			...
			for (
				...
				for (let item = 0; item < ADDRESS_BATCH; item += 1) {
					...
					used.add(index);
					retryFacts.push({
						...initialFact(index, options, payloadBytesPerFact),
						sequence: nextSequence++,
					});
					workloadMix.rowAddresses += 1;
				}
		...
		const reopenProbe = initialFact(0, options, payloadBytesPerFact);
		const reopenSamples = [
			timed(() => {
...
function main(): void {
	...
	const expectedCells = 2 * CANDIDATES.length * options.repetitions;
	const observedCellKeys = new Set(
		results.map(
			(result) => `${result.owner}/${result.candidate.id}/${result.repetition}`,
		),
	);
	const matrixComplete =
		results.length === expectedCells && observedCellKeys.size === expectedCells;
	const allCellsCorrect = results.every((result) => result.correct);
	const diagnosticStatus = {
		scope: 'legacy-bun-diagnostic' as const,
		...
		limitations: [
			...(matrixComplete
				? []
				: ['one or more owner, candidate, or repetition cells are missing']),
			...(allCellsCorrect ? [] : ['one or more conformance cells failed']),
			'this legacy runner does not consume the exact modular trace or the predeclared measurement-method pilot',
			'browser SQLite and OPFS qualification evidence is absent',
			'Cloudflare Durable Object SQLite qualification evidence is absent',
		],
	};
		...
		methodology: {
			...
			browserReplicaEvidence: {
				...
				requiredDimensions: [
					...
					'browser constraint and crash-recovery conformance for every candidate',
				],
			},
