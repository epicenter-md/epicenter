...
function openAccount(label: string) {
	...
	return {
		...
		device(name: string, principal = account) {
			...
			return {
				open: () => inside((replica) => replica.open(`device:${principal}`, ORIGIN)),
				write: (title: string, prose: string) =>
					inside((replica) => replica.write(title, prose)),
				report: (): Promise<ReplicaReport> => inside((replica) => replica.report()),
			};
		},
	};
}
...
describe('two devices on one account converge', () => {
	...
	it('CONTROL: a device on another account sees nothing', async () => {
		...
		const stranger = vault.device('stranger', `someone-else-${crypto.randomUUID()}`);
		await mine.open();
		await stranger.open();
	...
	it('both directions, and a third device catches up on arrival', async () => {
		...
		await tablet.open();
		await until('the tablet to catch up', async () =>
			(await tablet.report()).titles.length === 2,
		);
	...
	it('an upgrade with no bearer is refused', async () => {
		const response = await SELF.fetch(
			new Request(`${ORIGIN}/api/store/v1/sync?namespace=so.epicenter.storeprobe&cursor=0`, {
