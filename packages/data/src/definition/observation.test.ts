...
const DATA_ID = 'so.epicenter.test';
...
function row(tableName: string, index: number): RowAddress {
	return { dataId: DATA_ID, tableName, rowId: rowId(index) };
}
...
test('one batched commit produces one invalidation per logical table', () => {
	...
	dispatcher.subscribeTable(DATA_ID, 'notes', (i) => notes.push(i));
	dispatcher.subscribeTable(DATA_ID, 'tasks', (i) => tasks.push(i));
...
test('a repeated address is named once', () => {
	...
	dispatcher.subscribeTable(DATA_ID, 'notes', (i) => seen.push(i));
...
test('the same table name in two namespaces is two handles', () => {
	...
	dispatcher.subscribeTable(DATA_ID, 'notes', (i) => mine.push(i));
...
test('registration never fires and unsubscribing is complete', () => {
	const dispatcher = createInvalidationDispatcher();
	const seen: TableInvalidation[] = [];
	const stop = dispatcher.subscribeTable(DATA_ID, 'notes', (i) =>
		seen.push(i),
	);
	expect(seen).toEqual([]);
...
test('a gap heals every subscribed handle at the strongest honest scope', () => {
	...
	dispatcher.subscribeTable(DATA_ID, 'notes', (i) => notes.push(i));
	dispatcher.subscribeTable(DATA_ID, 'settings', (i) => settings.push(i));
...
test('a subscriber that throws does not cost another handle its invalidation', () => {
	...
	dispatcher.subscribeTable(DATA_ID, 'notes', () => {
		throw new Error('subscriber exploded');
	});
	dispatcher.subscribeTable(DATA_ID, 'tasks', (i) => survivor.push(i));
