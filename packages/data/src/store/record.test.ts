...
import { type DurableRecord, openDurableRecord } from './record.js';
import { openMemoryRecord } from './record-memory.js';

/**
 * Both records, one set of assertions.
 *
...
const implementations = [
	...
	{ label: 'memory', open: openMemoryRecord },
] as const satisfies readonly {
	label: string;
	open: (options: {
		name: string;
		floorBytes?: number;
	}) => DurableRecord | Promise<DurableRecord>;
}[];
	...
	describe(implementation.label, () => {
		...
		describe('folding', () => {
			...
			test('a sequence is never reused, not even across a fold', async () => {
				const name = freshName();
				const record = await implementation.open({ name });
				await record.read('app');
				await record.append('app', bytes(1));
				await record.fold('app', () => bytes(9));
				await record.append('app', bytes(3));
		...
		describe('the counters have to agree with disk, synchronously', () => {
			...
			test('a second record on one database is refused', async () => {
				...
				await expect(implementation.open({ name })).rejects.toBeDefined();
				first.close();
				const second = await implementation.open({ name });
				second.close();
			});
		...
		describe('shouldFold', () => {
			...
			test('gives the same answer before and after a reopen', async () => {
				const name = freshName();
				const first = await implementation.open({ name, floorBytes: 64 });
				await first.read('app');
				...
				const second = await implementation.open({ name, floorBytes: 64 });
...
// Below here the subject is the IndexedDB record specifically: what these
// exercise is a store that REFUSES, and nothing in the memory record can fail.
describe('durability', () => {
