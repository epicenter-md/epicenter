...
function applyOperation(state: LogicalState, operation: Operation): void {
	...
	const row: RowState = existing ?? { deleted: false, cells: {} };
...
function classify(
	state: LogicalState,
): Pick<ClientDump, 'rows' | 'quarantine' | 'tombstones'> {
	const rows: LogicalState = {};
	const quarantine: LogicalState = {};
	const tombstones: (keyof LogicalState & string)[] = [];
	for (const key of Object.keys(state).sort() as (keyof LogicalState &
		string)[]) {
		const row = state[key];
		if (row.deleted) {
			tombstones.push(key);
		...
		const fields = Object.keys(row.cells).sort();
		const valid =
			(table === 'notes' &&
		...
		(valid ? rows : quarantine)[key] = structuredClone(row);
	}
	return { rows, quarantine, tombstones };
}
...
export class RefServer {
	...
	private readonly actorHighWater = new Map<string, number>();
	private readonly canonical: LogicalState = {};
	...
	publishSnapshot(rowsPerChunk: number): SnapshotManifest {
		...
		const rows = (
			Object.entries(this.canonical) as [
				import('./protocol').RowKey,
				RowState,
			][]
		)
			.sort(([left], [right]) => left.localeCompare(right))
			.map(([key, row]): SnapshotRow => {
